<script lang="ts">
	...
	let activeSection = $state<'notifications' | 'usage' | 'codex' | 'about'>('notifications');
...
<div class="settings-tab">
	...
	<div class="settings-shell">
        <nav class="settings-nav" aria-label="Settings sections">
            ...
            {#if isTauri()}<button class:active={activeSection === 'codex'} aria-current={activeSection === 'codex' ? 'page' : undefined} onclick={() => activeSection = 'codex'}>Codex</button>{/if}
        ...
        {#if isTauri()}<div hidden={activeSection !== 'codex'}><CodexDesktopSupport /></div>{/if}
